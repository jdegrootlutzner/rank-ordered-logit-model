...
if (length(args)!=3) {
  stop("Provide the repository location [1], data location [2], and gender [3]", call.=FALSE)
}
...
for (year in 2019:2019){
  ...
  write(processed_data_list[[events]], paste("~/Desktop/pomona/", "event-list.txt",sep = ""))
  write(processed_data_list[[sorted_ids]], paste("~/Desktop/pomona/", "population.txt",sep = ""))
  ...
  while (e > epsilon){
    ...
    write(as.vector(post_mu_vals),
          paste("~/Desktop/pomona/", "theta-sigma-", sigma,".txt",sep = ""))
    e <- sum((prev_post_mu_vals- post_mu_vals)^2)
    
    if(e < epsilon){
      write(sqrt(-diag(solve(derivative_list[[D2]]))),
            paste("~/Desktop/pomona/", "theta-sigma-sigma-", sigma,".txt",sep = ""))
    }
    # write(e,
